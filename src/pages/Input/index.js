...
export default class Input extends Component {  
  constructor(props){
    super(props);
    this.state = {money: '', moneyConvertido:'Aguardando valor...', moeda1: this.props.moeda1, moeda2: this.props.moeda2, error: '', arrayMoedas: [], symbol1: '', symbol2: ''};
      ...
      fetch(url)
        .then(r => r.json()).then(json => {
          if(json.error){
            ...
            let s = this.state;
            s.moneyConvertido = (cotacao * parseFloat(this.state.money)).toFixed(2).toString();
  ...
  render(){
    console.log(this.state.symbol1);
    return(
      <ViewInput>
        <View style={{flexDirection: 'row', backgroundColor: "black", margin: 10, borderTopLeftRadius: 10, borderTopRightRadius: 10,
         borderBottomLeftRadius: 10, borderBottomRightRadius: 10, top: 15 }}>
          ...
          <Text style={{color: 'white', textAlign: 'center', margin: 10, fontSize: 12, fontWeight: 'bold'}}>{this.props.moeda1}</Text>           
        ...
        <View style={{flexDirection: 'row', backgroundColor: "black", margin: 10, borderTopLeftRadius: 10, borderTopRightRadius: 10,
         borderBottomLeftRadius: 10, borderBottomRightRadius: 10, top: 15 }}>
          <View style={{flexDirection: 'column', backgroundColor: "#FF6347", width: '20%', borderTopLeftRadius: 10,borderBottomLeftRadius: 10 }}>
          <Text style={{color: 'white', textAlign: 'center', margin: 10, fontSize: 12,fontWeight: 'bold'}}>{this.props.moeda2} </Text>
          </View>
